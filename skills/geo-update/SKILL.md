...
|-----------|-------------|
| Main skill | `~/.config/opencode/skills/geo/` |
| Sub-skills | `~/.config/opencode/skills/geo-*/` |
| Agents | `~/.claude/agents/geo-*.md` |
| Scripts | `~/.config/opencode/skills/geo/scripts/` |
| Schema templates | `~/.config/opencode/skills/geo/schema/` |
| Hooks | `~/.config/opencode/skills/geo/hooks/` |

Verify the installation exists by checking for `~/.config/opencode/skills/geo/SKILL.md`. If it does not exist, inform the user that GEO-SEO is not installed and suggest running the installer instead.
...
```bash
~/.config/opencode/skills/geo/.venv/bin/python3 -m pip install -r "$SOURCE_DIR/requirements.txt" --quiet
```
